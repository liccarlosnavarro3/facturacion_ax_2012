...
//		}
	...
	*/
	function send($data, $timeout=0, $response_timeout=30, $cookies=NULL) {
		//echo "este es el tiempo => ".$timeout;die();
		//echo "este es".$data;die();
	...
	*/
	function sendRequest($data, $cookies = NULL) {
	  ...
	  if ($this->io_method() == 'socket') {
		...
		//file_put_contents('nomames.txt', $this->fp);//die("");
		//echo "8===D";
		//echo 'wrote data to socket, length = ' . strlen($this->outgoing_payload);die();
	...
	function getResponse(){
	  ...
	  if ($this->io_method() == 'socket') {
	    ...
	    while (!isset($lb)){
			...
			//file_put_contents('nomames8=d.xml', $this->fp);die();
			//echo $tmp;die();
			$tmplen = strlen($tmp);
		...
		//echo $this->incoming_payload;die();
		// store header data
		$this->incoming_payload .= $data;
		//echo $this->incoming_payload;die();
		...
		do {
			...
			//echo "no se ni madre 2=>".$content_length;die();
			$strlen = 0;
...
//                    }
	  ...
	  //echo "llega";die();
		...
		//echo $this->response_status_line;die()
		...
		if (
			...
			($http_status >= 501 && $http_status <= 505)
		   	...
		   	echo "NO ME LA MAMES";die();
			$this->setError("Unsupported HTTP response status $http_status $http_reason (soapclient->response has contents of the response)");
			return false;
		}
		//echo "8==================D".$http_status;die();
		...
		if(isset($this->incoming_headers['content-encoding']) && $this->incoming_headers['content-encoding'] != ''){
			...
			if(strtolower($this->incoming_headers['content-encoding']) == 'deflate' || strtolower($this->incoming_headers['content-encoding']) == 'gzip'){
    			...
    			if(function_exists('gzinflate')){
					...
					$datalen = strlen($data);
					//echo $this->incoming_headers['content-encoding'];die();
...
class nusoap_client extends nusoap_base  {
	...
	*/
	function call($operation,$params=array(),$namespace='http://tempuri.org',$soapAction='',$headers=false,$rpcParams=null,$style='rpc',$use='encoded'){
		...
		//header('Content-type: text/xml');echo $soapmsg;die();
	...
	*/
	function send($msg, $soapaction = '', $timeout=0, $response_timeout=30) {
		$this->checkCookies();
		//header('Content-type: text/xml');echo $msg;die();
		// detect transport
		switch(true){
			// http(s)
